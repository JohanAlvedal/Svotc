...
### 7. MCP (Model Predictive Control) Implementation
**Problem:** Systemet kunde bara bromsa vid höga priser, inte optimera vid låga priser. MCP-läge fanns men användes aldrig.

**Lösning:**
- MCP aktiveras automatiskt när systemet INTE braking
- Fungerar i ALLA lägen (Smart, Vacation, ComfortOnly)
- Logik:
  ```yaml
  {% set mcp_active = phase not in ['ramping_up','holding'] and not guard %}
  
  {% if mcp_active and price_state == 'cheap' %}
...
**Fördelar:**
- **I Smart/Vacation-läge:** MCP värmer extra vid låga priser för att ladda huset inför dyra perioder
- **I ComfortOnly-läge:** MCP optimerar fortfarande vid billiga timmar (men bromsar aldrig vid höga priser)
- Reason code visar "MCP (predictive heating)" när aktiv
- Utnyttjar billiga timmar maximalt utan att kompromissa komfort
...
**Problem:** MCP-reason code fanns men användes aldrig korrekt.

**Lösning:**
- MCP aktiveras och visas korrekt när systemet optimerar vid låga priser
- Nya, tydligare status:
  - `SAFE_MODE` - vid sensorfel
  - `COMFORT_GUARD` - istället för vaga "PRICE_LIMITING_COMFORT"
  - `MCP` - Model Predictive Control aktivt (cheap heating)
...
{% set mapper = {
  ...
  'MCP': 'MCP (predictive heating)',
  ...
} %}
```

**MCP visas när:**
- Pris är under P30 (cheap)
- Systemet INTE braking (phase != ramping_up/holding)
- Comfort guard INTE aktiv
- Fungerar i ALLA lägen inklusive ComfortOnly
...
| MCP (cheap heating) | ❌ Saknas | ✅ heat_aggr*1.5 | Ny funktion |
| MCP i ComfortOnly | ❌ Fungerar ej | ✅ Aktivt | Optimering utan brake |
